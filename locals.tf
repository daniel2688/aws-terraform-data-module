locals {
  ...
  environment = lower(lookup(var.tags, "environment", "default-env"))
  project     = lower(lookup(var.tags, "project", "default-project"))
  owner       = lower(replace(lookup(var.tags, "owner", "default-owner"), " ", "-"))
